<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Create Rental Transaction') }}
        </h2>
    </x-slot>

    <div class="py-12" x-data="rentalTransactionForm()">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-sm">
                <div class="mx-auto py-4 px-4 sm:px-6 lg:px-8 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('rental-transactions.store') }}" @submit.prevent="submitForm">
                        @csrf
                                    ...
                                    <select id="facility_id" x-model="newFacility.facility_id" 
                                        ...
                                        @foreach($facilities as $facility)
                                            <option value="{{ $facility->id }}" data-price="{{ $facility->hourly_rate }}">
                                                {{ $facility->name }} (Rp{{ number_format($facility->hourly_rate, 0, ',', '.') }}/hour)
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
    ...
    <script>
        function rentalTransactionForm() {
            return {
                facilitiesData: @json($facilities->mapWithKeys(fn($f) => [$f->id => ['name' => $f->name, 'price' => $f->hourly_rate]])),
                ...
                getFacilityName(id) {
                    return this.facilitiesData[id]?.name || '';
                },
                
                getFacilityPrice(id) {
                    return this.facilitiesData[id]?.price || 0;
                },
